...
function getSourceVideoFps(video: HTMLVideoElement): number {
  try {
    ...
    const stream = anyVideo.captureStream?.() || anyVideo.mozCaptureStream?.();
    if (stream) {
...
export async function exportVideo(opts: ExportOptions): Promise<ExportResult> {
  ...
  const done = new Promise<void>((resolve, reject) => {
    ...
    recorder.onerror = (e) => {
      const errMessage = (e as any).error?.message || (e as any).message || 'Error desconocido del codificador';
